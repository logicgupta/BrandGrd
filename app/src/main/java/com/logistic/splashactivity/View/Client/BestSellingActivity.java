...
public class BestSellingActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        button.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                getDataFromEditText();
                progressDialog=new ProgressDialog(BestSellingActivity.this);
                progressDialog.setMessage("Uploading Data ....");
                progressDialog.setCancelable(false);
                progressDialog.show();
                if (productName.isEmpty()){

                    Toast.makeText(BestSellingActivity.this, "Please Enter Product Name !", Toast.LENGTH_SHORT).show();
                    progressDialog.dismiss();
                }
                else if (productDescription.isEmpty()){

                    Toast.makeText(BestSellingActivity.this, "Please Enter Product Description !", Toast.LENGTH_SHORT).show();
                    progressDialog.dismiss();
                }
                else  if (productOriginalPrice.isEmpty()){
                    Toast.makeText(BestSellingActivity.this, "Please Enter the Original Price !", Toast.LENGTH_SHORT).show();
                    progressDialog.dismiss();
                }
                else if (productDiscountPrice.isEmpty()){

                    Toast.makeText(BestSellingActivity.this, "Please Enter Product Discount Price !", Toast.LENGTH_SHORT).show();
                    progressDialog.dismiss();
                }
                else if (productQuantity.isEmpty()){
                    Toast.makeText(BestSellingActivity.this, "Please Enter the Product Quantity Available !", Toast.LENGTH_SHORT).show();
                    progressDialog.dismiss();
                }
                else if (uri1==null){

                    Toast.makeText(BestSellingActivity.this, "Please Select Image 1 from Gallery", Toast.LENGTH_SHORT).show();
                    progressDialog.dismiss();
                }
                else if (uri2==null){

                    Toast.makeText(BestSellingActivity.this, "Please Enter Image 2 from Gallery", Toast.LENGTH_SHORT).show();
                    progressDialog.dismiss();
                }
                else if (uri3==null){

                    Toast.makeText(BestSellingActivity.this, "Please Enter Image 3 from Gallery", Toast.LENGTH_SHORT).show();

                    progressDialog.dismiss();
                }
                else if (uri4==null){
                    Toast.makeText(BestSellingActivity.this, "Please Enter Image 4 from Gallery", Toast.LENGTH_SHORT).show();
                    progressDialog.dismiss();
                }
                else {
                    viewModel= ViewModelProviders.of(BestSellingActivity.this).get(ProductViewModel.class);

                    viewModel.bestDealProduct(BestSellingActivity.this,uri1.toString(),uri2.toString(),uri3.toString(),uri4.toString()
                            ,productName,productDescription,
                            productOriginalPrice
                            ,productDiscountPrice,productQuantity,pinCode
                            ,sellerName,qty);

                    viewModel.bestRepogetStatus().observe(BestSellingActivity.this, new Observer<Boolean>() {
                        @Override
                        public void onChanged(Boolean aBoolean) {

                            if (aBoolean){
                                Toast.makeText(BestSellingActivity.this, "Success ! ", Toast.LENGTH_SHORT).show();
                                progressDialog.dismiss();
                            }
                            else {
                                Toast.makeText(BestSellingActivity.this, "Failed !", Toast.LENGTH_SHORT).show();
                                progressDialog.dismiss();
                            }
                        }
                    });
